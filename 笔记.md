...
const b:string = '<span style="color:red;">我是文字</span>'
//不支持组件
...
### button
<div @click="father"> 
  <button @[event].stop="touch">dian</button>
</div>
 const touch=()=>{
  console.log('1')
}
...
const aa = ref('panchen') //响应式
下面也跟着变

## ref
ref() 定义响应式数据
ref(xx) 加工的数据 RefImpl{。。。value}
基本类型数据 .value=      Object.definPrototype() 的get和set
对象         .value.xx     vue3的reactive 
.value{key1：'value1',key2:'value2'}=我们的reactive加工的数据
isRef判断是不是ref对象

shallowRef 浅层次 .value是响应式 .value = 
...
<button @click="shallow">修改</button>

const name = ref({myname:'panchen'})
const name1 = shallowRef({myname:'panchen'})
const change =()=>{
  name.value.myname = '潘晨'
}
const shallow =()=>{
  name1.value = {
    myname:'潘晨1'
...
const change =()=>{
  name.value.myname = '潘晨'
  name1.value.myname='潘晨'
}放一起都变 shallowRef被影响
```

## reactive
定义对象类型的响应式数据 返回proxy对象
reactive({
